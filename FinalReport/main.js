...
(function(){
  var MAXFONTSIZE, DEFAULTFONTSIZE, MAXFONTSIZEFORSIDEHEADING, ref$, TC, JQ, m, _, TM, Doc, MDoc, GetRidOfNewLine, KeepOnlyString, AnchorStyle, ListOfReferences, ListOfFigRef, ListOfEquRef, ListOfHref, ListOfFigCaption, ListOfEqu, ReplaceFigCaption, CreateListOfFigCaption, ReplaceRef, EmptyTest, CreateListOfHref, SeperateByTag, FindListOfHeading, CreateNumberList, ReplaceWithNum, FontSizeBasedOnH, Run, HeadingList, Run1, Curate, Mod, WithOnlyNames, Jm, CreateSideBar, CreateIndexPage, app, config, Width, triangleShape, mediaQueryList, WhenPrint;
  MAXFONTSIZE = 50;
  DEFAULTFONTSIZE = 20;
  ...
  triangleShape = {
    ...
    height: "0",
    top: "45%",
    borderTop: app.Height * 0.06 + "px solid transparent",
    borderBottom: app.Height * 0.06 + "px solid transparent",
    borderLeft: app.Height * 0.06 + "px solid rgb(0,0,0)",
    opacity: 0.4,
    position: "fixed",
    zIndex: 2,
    cursor: "pointer"
  ...
  app.view = function(ctrl){
    ...
    TriangleClick = function(){
      var Val, Rotation, Signal, Amount;
      Val = [0, 21];
      Rotation = [360, 180];
      Signal = ctrl.SideBarHidden;
      TM.to(app.SideBar, 0.5, {
        marginLeft: (-Val[(Signal + 1) % 2] - 0.4) + "%"
      });
      Amount = Val[Signal % 2];
      TM.to(app.LatexDoc, 0.5, {
        marginLeft: (Amount + 5) + "%",
        width: (100 - Amount - 10 + 3 * Signal) + "%"
      });
      TM.to(app.InnerTriangle, 0.5, {
      ...
      setTimeout(function(){
        return TM.to(app.InnerTriangle, 0.5, {
          opacity: 0.4
        });
      }, 200);
      TM.to(app.OuterTriangle, 0.5, {
