...
GlobalIm = [];
GlobalX = [];
labels = cellstr( num2str([1:50]') );  %' # labels correspond to their order\
...
% for U = 0:50
	...
	h = (-2*3.14*rho*(U^2)*alpha)/kalpha0;
	...
	E = eig(dq_dt);
	GlobalX = [GlobalX ,real(E)];
	GlobalIm = [GlobalIm ,imag(E)];
...
% Create title
	% title('Eigen Value for linearized System');
% end

createfigure (GlobalIm,GlobalX,labels); 
